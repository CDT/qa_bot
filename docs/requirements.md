...
- Model-agnostic: switching providers (OpenAI, Anthropic, local, etc.) must require only configuration changes, not code changes.
...
- Storage: plain files on disk — no database required for the KB itself.
...
Revisit this decision only if recall demonstrably breaks on paraphrased or semantic questions where user wording doesn't overlap with filenames or file contents.
...
- **Chat**: ask questions and receive answers from the bot, ideally with citations back to the source file(s) used.
